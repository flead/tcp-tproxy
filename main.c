...
static bool tproxy_trans = false; 
...
static void 
die_usage(void) {
	fputs("Usage: tcp-proxy [ -4 | -6 ] [ -f ] [ -l ] [ -t [ -T ]] [ -L listen address ] localport host [ remoteport ]\n", stderr);
	exit(1);
}


static const char *
getxinfo_strerr(int err) {
	const char *errstr;
...
static void 
xgetnameinfo(const struct sockaddr *sa, socklen_t salen, char *host,
...
static void 
ipaddrtostr(const struct sockaddr *sa, socklen_t salen,
	      	char *resbuf, size_t reslen, char *port, size_t plen) {
	xgetnameinfo(sa, salen, resbuf, reslen, port, plen, NI_NUMERICHOST|NI_NUMERICSERV);
}


static void 
logendpoints(int dest, int origin) {
...
static int 
sock_listen_tcp(const char * const listenaddr, const char * const port) {
	int sock;
...
static int
connect_fastopen(int txfd, char *buf, int blen, 
		const struct sockaddr *dest_addr, socklen_t alen) {
	int ret;
...
static int 
sock_connect_tcp(const char * const remoteaddr, const char * const port, int rs) {
	char buf[1024];
	int buflen;
	int sock;
	struct addrinfo hints = {
...
static size_t 
do_write(const int fd, char *buf, const size_t len) {
	size_t offset = 0;
...
static void 
copyfd_io(int fd_zero, int fd_one) {
...
int 
main_loop(int listensock, const char *host, const char *port) {
	struct sockaddr sa;
	socklen_t salen = sizeof(sa);
